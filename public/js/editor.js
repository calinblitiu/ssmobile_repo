...
(function( $ ){
	...
	var methods = {
		...
		init : function( options )
		{
			...
			var menuItems = { 'texteffects': true,
		...
		createMenuItem: function(itemSettings, options, returnElement){
			...
			if(itemSettings["select"]){
				...
				var menuWrapElement = $("<a/>",{href:'javascript:void(0)', class:'btn btn-default '+itemSettings["className"]});
				var menuElement = $("<i/>");
				if(itemSettings["icon"])
					menuElement.addClass(itemSettings["icon"]);
				else
			...
			if(itemSettings["custom"]){
				...
				menuWrapElement.bind(action, function(){ methods.setTextFormat.apply(this); $(this).toggleClass(' click'); });
			}
			menuWrapElement.attr('title', itemSettings['tooltip']);
