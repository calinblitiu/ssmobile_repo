...
<script  type="text/javascript">
  $(document).ready(function(){
    ...
    clipboard.on('success', function(e) {
      sweetAlert('Done', 'The url has been copied to clipboard and you can share it with friends', 'success');
      e.clearSelection();
    });

    $('.stream_info_list a').click(function(e){
      e.stopPropagation();
    });
  $('.toggleStreams').click(function(e){
    e.preventDefault();
    no = $(this).attr('rel');
    console.log(no);
    if($(this).hasClass('active')){
      $(this).html('Show Streams (<span>'+no+'</span>)');
      $(this).removeClass('active');
      $(this).parent().parent().siblings('.stream_block').slideToggle();
    } else {
      $(this).html('Show Streams (<span>'+no+'</span>)');
      $(this).addClass('active');
      $(this).parent().parent().siblings('.stream_block').slideToggle();
  ...
  {
    $('form#postComment').toggle();
  }

  // show comment form
  function toggleAddStreamCommentForm(el)
  {
    el.next().toggle();
  }
</script>
